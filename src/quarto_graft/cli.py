...
from .constants import PROTECTED_BRANCHES, ROOT, TRUNK_TEMPLATES_DIR, GRAFT_TEMPLATES_DIR
...
def graft_destroy(
    ...
    if branch not in all_branches and not typer.confirm(
...
def main_callback(ctx: typer.Context) -> None:
    ...
    if len(parts) == 2:
        ...
        if group == "trunk" and command == "init":
            ...
            graft_create(name=None, template=None, branch_name=None, push=True)
            ...
            if choices:
                ...
                branch = questionary.text("Enter branch name to destroy (e.g. graft/chapter1):").ask()
