...
def _export_from_worktree(
    ...
    try:
        with managed_worktree(ref, worktree_name) as wt_dir:
            ...
            docs_dir = wt_dir / "docs"
            if not docs_dir.exists():
                raise RuntimeError(f"[{branch}] {wt_dir} has no docs/ directory")

            cfg = load_quarto_config(docs_dir)
            section_title = derive_section_title(cfg, branch)

            src_relpaths = collect_exported_relpaths(docs_dir, cfg)
        ...
        if last_good_sha and _branch_exists(last_good_sha):
            ...
            try:
                sha, title, exported_relpaths, exported_dest_paths = _export_from_worktree(
                    ...
                    branch_key=branch_key,
                    ref=last_good_sha,
                    worktree_name=f"{branch_key}-lastgood",
                    inject_warning=True,
                    warn_head_sha=None,
